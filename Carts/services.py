...
async def add_to_cart(db: _orm.Session, book_id: int, user_id: int):
    user = db.query(_UserModel.User).filter(_UserModel.User.id == user_id).first()
    ...
    cart = db.query(_models.Cart).filter(_models.Cart.user_id == user_id).first()
        
    book = db.query(_BookModel.Book).filter(_BookModel.Book.id == book_id).first()
